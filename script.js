...
// ... (todo o seu código existente antes desta parte) ...

// --- Funções de Acessibilidade (agora para tema manual/detecção) ---

// Esta função agora alterna explicitamente entre dark-theme e light-theme
// Ela é acionada pelo botão de configurações, permitindo sobrescrever a preferência do sistema
function toggleThemeManually() {
    const widget = document.querySelector('.agenda-widget');

    if (widget.classList.contains('dark-theme')) {
        // Se já está no tema escuro manual, volta para o tema claro padrão
        widget.classList.remove('dark-theme');
        widget.classList.add('light-theme'); // Adiciona light-theme para sobrescrever prefers-color-scheme: dark
        // Ajusta a modal também
        eventModal.querySelector('.modal-content').classList.remove('high-contrast-modal');
        alert('Tema claro ativado manualmente.');
    } else if (widget.classList.contains('light-theme')) {
        // Se está no tema claro manual, volta para o modo automático (removendo light-theme)
        widget.classList.remove('light-theme');
        // A modal voltará ao padrão ou ao tema escuro se o sistema preferir
        updateModalTheme(); // Chamamos uma nova função para verificar e aplicar
        alert('Tema automático ativado (conforme a preferência do seu sistema).');
    }
    else {
        // Se está no modo automático (sem light-theme ou dark-theme), ativa o dark-theme manual
        widget.classList.add('dark-theme');
        // Ajusta a modal também
        eventModal.querySelector('.modal-content').classList.add('high-contrast-modal');
        alert('Tema escuro ativado manualmente.');
    }
    // Salva a preferência de tema do usuário no localStorage (opcional, para persistir a escolha manual)
    saveThemePreference();
}

// Nova função para verificar o tema e aplicar à modal (chamada ao abrir a modal e ao mudar o tema manual)
function updateModalTheme() {
    const widget = document.querySelector('.agenda-widget');
    const modalContent = eventModal.querySelector('.modal-content');

    // Se o widget está em modo escuro (seja por detecção automática ou manualmente)
    if (window.matchMedia('(prefers-color-scheme: dark)').matches && !widget.classList.contains('light-theme') || widget.classList.contains('dark-theme')) {
        modalContent.classList.add('high-contrast-modal');
    } else {
        modalContent.classList.remove('high-contrast-modal');
    }
}

// Salva a preferência de tema manual do usuário
function saveThemePreference() {
    const widget = document.querySelector('.agenda-widget');
    let theme = 'auto'; // Padrão
    if (widget.classList.contains('dark-theme')) {
        theme = 'dark';
    } else if (widget.classList.contains('light-theme')) {
        theme = 'light';
...
// Carrega a preferência de tema do usuário ao iniciar a página
function loadThemePreference() {
    ...
    // Se 'auto' ou nada, ele simplesmente segue o prefers-color-scheme do CSS
}


// Event listener para o botão de configurações (agora aciona a alternância de tema manual)
document.querySelector('.settings-button').addEventListener('click', toggleThemeManually);


// --- Adicionar a chamada de updateModalTheme na openModal() ---
// Localize a função openModal() no seu script.js e SUBSTITUA o bloco que eu te dei anteriormente
// por esta ÚNICA LINHA no lugar:
/*
function openModal(item = null) {
    // ... (resto do seu código existente na openModal) ...

    // NOVO CÓDIGO A SER ADICIONADO AQUI, SUBSTITUINDO O ANTERIOR
    updateModalTheme(); // <--- Chamada da nova função de verificação de tema da modal
    // --- FIM DO NOVO CÓDIGO ---
...
// --- Inicialização (Adicionar carregamento de preferência de tema) ---
document.addEventListener('DOMContentLoaded', () => {
    loadEvents(); // Carrega os eventos ao iniciar
    loadThemePreference(); // Carrega a preferência de tema do usuário
    updateDateDisplay(); // Renderiza a agenda para a data atual
    updateModalTheme(); // Garante que a modal esteja com o tema correto ao carregar
});
